...
IAM에서 실행 역할을 따로 만들지 않고 Lambda 함수 생성 과정에서 기본 실행 역할을 함께 생성합니다.

1. AWS 콘솔 위쪽 검색창에 `Lambda`를 입력하고 검색 결과에서 **Lambda**를 선택합니다.
2. 화면 오른쪽 위의 리전이 **서울**인지 확인합니다.
3. 왼쪽 메뉴에서 **함수**를 선택합니다.
4. 오른쪽 위의 **함수 생성**을 누릅니다.
5. 함수 생성 방식에서 **새로 작성**을 선택합니다.
6. **기본 정보**에 다음 값을 입력합니다.
...
7. **기본 실행 역할 변경**을 펼칩니다.
8. **실행 역할**에서 **기본 Lambda 권한을 가진 새 역할 생성**을 선택합니다.
9. 나머지 항목은 기본값으로 두고 **함수 생성**을 누릅니다.
10. 함수 상세 화면이 열리고 성공 메시지가 표시될 때까지 기다립니다.

### 4. 배포 ZIP 파일 만들기

Lambda에는 Python 파일을 하나씩 올리지 않고, 실행에 필요한 파일과 폴더를 하나의 ZIP 파일로 만들어 업로드합니다. 다만 현재 로컬 추천기는 `sentence-transformers`와 `numpy`를 사용하므로 기존의 표준 라이브러리 전용 ZIP 배포 절차에는 그대로 적용할 수 없습니다. 임베딩 추천을 활성화한 운영은 로컬 환경을 사용합니다.
...
### 5. Lambda 코드 배포 및 실행 설정

앞에서 만든 `news-digest` 함수에 ZIP 파일을 업로드하고 실행 설정을 변경합니다.

#### 5-1. Python ZIP 파일 업로드하기
...
뉴스 선별 범위는 기본 한국 시간 기준으로 일반일에는 전날 오전 7시 초과부터 당일 오전 7시 이하입니다. 월요일은 주말을 포함해 금요일 오전 7시 초과부터 월요일 오전 7시 이하를 대상으로 합니다.
...
로컬 의미 추천은 `numpy`와 `sentence-transformers`를 사용합니다. 최초 모델 로딩 시 Hugging Face에서 모델을 내려받으며, 이후에는 로컬 캐시를 사용합니다.
...
Windows PC가 켜져 있을 때 매일 오전 7시 28분에 뉴스 수집과 메일 발송을 자동 실행하려면 다음과 같이 등록합니다. 등록하기 전에 PowerShell에서 실제 발송 명령이 정상적으로 끝나는지 먼저 확인합니다.
...
위 경로의 `C:\path\to\news-digest-source`는 이 프로젝트가 저장된 실제 절대 경로로 바꿉니다. 시험 수신자에게 먼저 확인하려면 `.env`의 `TEST_MODE=true`와 `TEST_RECIPIENTS`를 설정한 상태에서 실행합니다.

#### 4-1. 작업 만들기

1. Windows 시작 메뉴에서 **작업 스케줄러**를 검색하여 실행합니다.
2. 오른쪽 **작업** 영역에서 **작업 만들기**를 누릅니다. **기본 작업 만들기**가 아니라 **작업 만들기**를 사용해야 실행 시작 위치와 세부 조건을 모두 지정할 수 있습니다.
3. **일반** 탭에서 다음과 같이 설정합니다.

   | 항목 | 설정값 |
   | --- | --- |
   | 이름 | `이노뎁 뉴스 다이제스트` |
   | 설명 | `매일 오전 7시 28분 뉴스 수집 및 이메일 발송` |
   | 보안 옵션 | 필요에 따라 **사용자가 로그온할 때만 실행** 또는 **사용자의 로그온 여부에 관계없이 실행** 선택 |

   - **사용자가 로그온할 때만 실행**: 저장할 때 Windows 암호가 필요하지 않습니다. 예약 시간에 해당 계정이 로그인된 상태여야 합니다. `Win + L`로 화면만 잠근 상태는 로그아웃이 아니므로 정상 실행됩니다.
   - **사용자의 로그온 여부에 관계없이 실행**: 로그아웃 상태에서도 실행할 수 있지만 저장할 때 현재 Windows 계정의 로그인 암호를 입력해야 합니다. Windows Hello PIN이 아니라 계정 암호를 사용합니다.

4. **트리거** 탭에서 **새로 만들기**를 누르고 다음과 같이 설정한 뒤 **확인**을 누릅니다.
...
5. **동작** 탭에서 **새로 만들기**를 누르고 **프로그램 시작**을 선택합니다. 각 입력란에는 다음 값을 지정합니다.
   ...
   세 경로의 `C:\path\to\news-digest-source`를 모두 같은 실제 프로젝트 경로로 바꿉니다. **시작 위치**가 빠지면 프로그램이 `.env`를 찾지 못할 수 있으므로 반드시 입력합니다. 입력란에는 예시의 백틱 문자를 넣지 않습니다.

6. **조건** 탭에서 실행 환경에 맞게 설정합니다.

   - 절전 상태에서도 예약 시간에 실행하려면 **이 작업을 실행하기 위해 절전 모드 종료**를 체크합니다.
   - 노트북을 배터리로 사용할 때도 실행해야 한다면 **컴퓨터의 AC 전원이 켜져 있는 경우에만 작업 시작**을 해제합니다.
   - 노트북 덮개를 닫았을 때 최대 절전 모드로 들어가거나 전원이 꺼지면 작업을 실행할 수 없으므로 Windows 전원 설정도 함께 확인합니다.

7. **설정** 탭에서 다음 항목을 권장합니다.

   - **예약된 시작 시간을 놓친 경우 가능한 대로 작업 시작** 체크
   - **작업이 실패하는 경우 다시 시작 간격**을 `5분`, 다시 시작 시도 횟수를 `3회`로 설정
   - **요청할 때 작업이 실행되도록 허용** 체크
   - 작업이 이미 실행 중일 때 적용할 규칙은 **새 인스턴스를 시작하지 않음** 선택

8. **확인**을 눌러 저장합니다. **사용자의 로그온 여부에 관계없이 실행**을 선택했다면 계정 정보 입력 창에 현재 Windows 계정의 로그인 암호를 입력합니다.

#### 4-2. 등록 결과 시험

1. 작업 스케줄러 왼쪽에서 **작업 스케줄러 라이브러리**를 선택합니다.
2. 등록한 **이노뎁 뉴스 다이제스트**를 찾아 마우스 오른쪽 버튼으로 누른 뒤 **실행**을 선택합니다.
3. 작업이 끝난 뒤 `F5`를 눌러 새로 고침하고 다음 내용을 확인합니다.

   - **상태**가 `준비`로 돌아왔는지 확인합니다.
   - **마지막 실행 결과**가 `0x0`이면 정상 종료입니다.
   - `0x41301`은 아직 실행 중이라는 뜻이므로 수집이 끝날 때까지 기다린 후 다시 새로 고침합니다.
   - `0x1` 등 다른 결과가 나오면 프로그램 경로, `main.py`, 시작 위치, `.env` 설정을 다시 확인합니다.

4. `.env`에서 지정한 시험 수신자에게 이메일이 도착했는지 확인합니다. 정상 동작을 확인한 후 실제 발송으로 전환하려면 `TEST_MODE=false`와 `REAL_RECIPIENTS`를 확인합니다.

PC 전원이 꺼져 있으면 작업 스케줄러가 실행할 수 없습니다. 화면 잠금은 괜찮지만, 안정적인 자동 발송을 위해 예약 시각에는 PC를 켜 두고 절전 해제 옵션을 확인합니다.
